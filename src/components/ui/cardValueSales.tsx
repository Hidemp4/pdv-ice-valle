...
interface CardValueSalesProps {
  salesData: mockSale[]; // Recebe os dados das vendas
}

// Função helper para calcular valores
const calculateSalesValues = (salesData: mockSale[]) => {
  const today = new Date();
  const currentMonth = today.getMonth();
  const currentYear = today.getFullYear();

  // Formatar data de hoje para comparação (DD/MM formato)
  ...
  const todaySales = salesData.filter(sale => {
    // Extrair apenas DD/MM da data da venda para comparar
    const saleDateParts = sale.date.split(' ')[0]; // "02/09" da string "02/09 21:56:04"
    return saleDateParts === todayFormatted;
  });

  const monthSales = salesData.filter(sale => {
    // Converter data da venda para Date para comparar mês/ano
    const [datePart] = sale.date.split(' ');
    const [day, month, year = currentYear] = datePart.split('/').map(Number);
    const saleDate = new Date(year, month - 1, day); // month - 1 porque Date usa 0-indexado

    return saleDate.getMonth() === currentMonth && saleDate.getFullYear() === currentYear;
  });

  // Converter string "R$12,00" para número
  const parsePrice = (priceString: string): number => {
    return parseFloat(priceString.replace('R$', '').replace(',', '.'));
  };

  const totalDay = todaySales.reduce((sum, sale) => sum + parsePrice(sale.total), 0);
  const totalMonth = monthSales.reduce((sum, sale) => sum + parsePrice(sale.total), 0);
...
const CardValueSales: React.FC<CardValueSalesProps> = ({ salesData }) => {
  // Calcular os valores usando a função helper
  const { salesDay, salesMonth } = calculateSalesValues(salesData);

  const salesDayValue = salesDay.toFixed(2);
  const salesMonthValue = salesMonth.toFixed(2);
  ...
  const sales: Sale[] = [
    {
      ...
      icon: (
        <svg width="28" height="28" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <path fillRule="evenodd" clipRule="evenodd" d="M12.25 2C6.72715 2 2.25 6.47715 2.25 12C2.25 17.5228 6.72715 22 12.25 22C17.7728 22 22.25 17.5228 22.25 12C22.25 6.47715 17.7728 2 12.25 2ZM3.75 12C3.75 7.30558 7.55558 3.5 12.25 3.5C16.9444 3.5 20.75 7.30558 20.75 12C20.75 16.6944 16.9444 20.5 12.25 20.5C7.55558 20.5 3.75 16.6944 3.75 12Z" fill="#343C54" />
        </svg>
      ),
    },
    {
      ...
      {sales.map((sale) => (
        ...
        >
          <div className="space-y-2">
            ...
            <h2 className="text-3xl font-medium">R${sale.value}</h2>
